...
/*   Updated: 2025/11/04 00:54:20 by theo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_vec2	spheric(t_vec3 vec3, t_fdf *fdf, double y, double x)
{
	...
	point = apply_transformation(point, fdf, 0);
	screen_pos.x = (point.x *(fdf->settings->scale * 0.1))
		+ fdf->settings->offset_x;
	screen_pos.y = (point.y * (fdf->settings->scale * 0.1))
		+ fdf->settings->offset_y;
	return (screen_pos);
}
